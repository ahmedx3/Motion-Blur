...
export default class MotionBlurScene extends Scene {
    ...
    public start(): void {
        ...
        this.objects['moon'] = {
            ...
            tint: [1,1,1,1],
            currentModelMatrix: mat4.fromRotationTranslationScale(mat4.create(), quat.fromEuler(quat.create(), 0, 0, 22.5), vec3.fromValues(0, 10, 0), vec3.fromValues(5, 5, 5)),
            previousModelMatrix: mat4.create()
        };
    ...
    public draw(deltaTime: number): void {
        ...
        this.V_prev = this.camera.ViewMatrix;
        this.P_prev = this.camera.ProjectionMatrix;
        ...
        if(!this.paused){ // If paused, we will skip updating time and the matrices
            ...
            this.objects['house'].currentModelMatrix = mat4.fromRotationTranslationScale(mat4.create(), quat.fromEuler(quat.create(), 0, 0, 0), vec3.fromValues(-10, 0, -10 - 10 + 20*triangle(this.time/1000)), vec3.fromValues(1, 1, 1));
            this.objects['moon'].currentModelMatrix = mat4.fromRotationTranslationScale(mat4.create(), quat.fromEuler(quat.create(), 0, 360*this.time/1000, 0), vec3.fromValues(-100, 100, -100), vec3.fromValues(50, 50, 50))
        }
        ...
        this.gl.bindFramebuffer(this.gl.FRAMEBUFFER, this.frameBuffer);
        {
            ...
            for(let key in this.objects){
                ...
                program.setUniformMatrix4fv("M_Prev", false, obj.previousModelMatrix);
        ...
        this.gl.bindFramebuffer(this.gl.FRAMEBUFFER, null);
        {
            ...
            if(this.motionBlurEnabled){ // If motion blur is enabled, draw fullscreen using the motion blur shader
                ...
                program.setUniformMatrix4fv("P_matPrev_i", false, mat4.invert(mat4.create(), this.P_prev));
